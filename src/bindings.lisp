...
(defun write-init-function (name linkage stream &optional (initialize-lisp-args nil))
  ...
  (format stream "  initialize_thread();")
...
(defun build-bindings (library directory &key (omit-init-function nil)
  ...
  (let* ((c-name (library-c-name library))
    ...
    (with-open-file (stream (merge-pathnames source-name directory)
      ...
      (format stream "void ~A (*answer_alien_calls_forever)(void *args);~%" linkage)
      (format stream "void ~A initialize_thread(void) { void *alien_fiber = ConvertThreadToFiber(NULL); struct fiber_args args; args.alien_fiber = alien_fiber; void *lisp_fiber = CreateFiber(0, answer_alien_calls_forever, &args); args.lisp_fiber = lisp_fiber; SwitchToFiber(lisp_fiber); }~%" linkage)
      (dolist (api (library-apis library))
